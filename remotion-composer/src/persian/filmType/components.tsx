...
const CompactFilmField: React.FC<{rect:Rect;format:PersianFormat;design:PersianDesignSnapshot;color:string;alpha:number;opacity:number;travel:number}>=({rect,format,design,color,alpha,opacity,travel})=>{
 ...
 const layers=useMemo(()=>featherLayers(cfg.featherPx,cfg.steps,curve).map(layer=>({...layer,path:compactPath(w,h,cfg.paddingPx,layer.expand,cfg.exponent)})),[w,h,cfg.paddingPx,cfg.featherPx,cfg.steps,cfg.exponent,curve]);
...
export const PersianFilmTypeMoment: React.FC<{
  ...
  const modern=p.profileVersion === "2.4.0" || p.profileVersion === "2.5.0",lifeAt=modern?gentleLife:filmLife;
  ...
  return <AbsoluteFill data-film-type-moment={moment.id} data-film-type-placement={layout.placement} style={{pointerEvents:"none"}}>
    {modern ? <CompactFilmField rect={layout.rect} format={format} design={design} color={dark?p.contrast.darkField:p.contrast.lightField}
...
export const PersianFilmTypeWatermark: React.FC<{
  ...
  const opacity=p.profileVersion === "2.4.0" || p.profileVersion === "2.5.0" ? gentleLife(seconds-entry.startSeconds,entry.endSeconds-entry.startSeconds,0,p.watermark.transitionSeconds,p.watermark.transitionSeconds).opacity
