...
def _bootstrap(tmp_path: Path, *, project_id: str = "run", input_kind: str = "script", input_value: str = "متن"):
    calls: list[str | None] = []
...
def _advance_to(tmp_path: Path, target: str, *, project_id: str = "run") -> dict:
    ...
    while state.get("next_phase") != target:
        ...
        state = complete_phase(project_id, phase, pipeline_dir=tmp_path, now=BASE)
    return state
...
def test_bootstrap_supports_all_four_input_classes_and_opens_backlot(tmp_path):
    audio = tmp_path / "source.wav"
    audio.write_bytes(b"audio")
    cases = [
        ("narration_path", str(audio), "transcribe_existing_narration"),
        ("script", "متن تاییدشده", "use_approved_persian_script"),
        ("raw_text", "یادداشت خام", "author_persian_narration"),
        ("english_article", "English source", "translate_and_author_persian_narration"),
    ]
    for index, (kind, value, mode) in enumerate(cases):
        root = tmp_path / f"projects-{index}"
        state, calls = _bootstrap(root, project_id=f"run-{index}", input_kind=kind, input_value=value)
        assert calls == [f"run-{index}"]
        assert state["completed_phases"] == list(PHASES[:3])
        assert state["next_phase"] == "prepare_narration"
        assert state["input"]["kind"] == kind
        assert state["input"]["prepare_mode"] == mode
        marker = json.loads((root / f"run-{index}" / "project.json").read_text(encoding="utf-8"))
        assert marker["pipeline_type"] == "persian-footage"
        if kind == "narration_path":
            assert Path(state["input"]["source_path"]) == audio.resolve()
        else:
            assert Path(state["input"]["source_path"]).is_relative_to((root / f"run-{index}").resolve())
...
def test_bootstrap_refuses_existing_project_without_opening_backlot(tmp_path):
    ...
    with pytest.raises(PersianVideoWorkflowError, match="refusing to reuse"):
        bootstrap_persian_video(
            title="Run", input_kind="script", input_value="متن", project_id="run",
...
def test_read_allowlist_permits_current_source_and_contracts_but_not_siblings(tmp_path):
    ...
    state, _ = _bootstrap(projects, input_kind="narration_path", input_value=str(source))
    ...
    assert assert_read_allowed(state, source) == source.resolve()
    required_contracts = [
...
def test_phase_order_is_exact_and_skips_are_refused(tmp_path):
    assert PHASES == (
        "validate_input", "create_project", "open_backlot", "prepare_narration",
    ...
    with pytest.raises(PersianVideoWorkflowError, match="must be attempted"):
        complete_phase("run", "prepare_narration", pipeline_dir=tmp_path, now=BASE)
    record_phase_attempt("run", "prepare_narration", pipeline_dir=tmp_path, now=BASE)
    state = complete_phase("run", "prepare_narration", pipeline_dir=tmp_path, now=BASE)
...
def test_retry_budget_is_deterministic(tmp_path):
    ...
    for expected in range(1, 5):
        state = record_phase_attempt("run", "prepare_narration", pipeline_dir=tmp_path, now=BASE)
        assert state["attempts"]["prepare_narration"] == expected
...
def test_send_back_budget_and_resume_preserve_durable_counters(tmp_path):
    ...
    record_phase_attempt("run", "prepare_narration", pipeline_dir=tmp_path, now=BASE)
    complete_phase("run", "prepare_narration", pipeline_dir=tmp_path, now=BASE)
    state = request_send_back("run", "prepare_narration", reason="revise", pipeline_dir=tmp_path, now=BASE)
    assert state["send_backs"] == 1
    assert state["attempts"]["prepare_narration"] == 1
    record_phase_attempt("run", "prepare_narration", pipeline_dir=tmp_path, now=BASE)
    complete_phase("run", "prepare_narration", pipeline_dir=tmp_path, now=BASE)
    state = request_send_back("run", "prepare_narration", reason="revise again", pipeline_dir=tmp_path, now=BASE)
    assert state["send_backs"] == 2
    record_phase_attempt("run", "prepare_narration", pipeline_dir=tmp_path, now=BASE)
    complete_phase("run", "prepare_narration", pipeline_dir=tmp_path, now=BASE)
    with pytest.raises(PersianVideoWorkflowError, match="send-back budget exhausted"):
        request_send_back("run", "prepare_narration", reason="too many", pipeline_dir=tmp_path, now=BASE)
    ...
    assert resumed["attempts"]["prepare_narration"] == 3
...
def test_wall_time_expires_until_resume_starts_a_new_window(tmp_path):
    ...
    with pytest.raises(PersianVideoWorkflowError, match="wall-time budget exceeded"):
        record_phase_attempt("run", "prepare_narration", pipeline_dir=tmp_path, now=expired)
    resumed = resume_workflow("run", pipeline_dir=tmp_path, backlot_opener=lambda _: 0, now=expired)
    state = record_phase_attempt("run", "prepare_narration", pipeline_dir=tmp_path, now=expired)
    assert state["attempts"]["prepare_narration"] == 1
...
def test_asset_actions_and_send_back_obey_session_wall_time(tmp_path):
    ...
    with pytest.raises(PersianVideoWorkflowError, match="wall-time budget exceeded"):
        request_send_back(
            "run", "prepare_narration", reason="late", pipeline_dir=tmp_path, now=expired,
        )
...
def test_valid_digest_bound_candidate_stops_workflow_at_awaiting_human(tmp_path, monkeypatch):
    ...
    with pytest.raises(PersianVideoWorkflowError, match="already stopped"):
        request_send_back(
            "run", "prepare_narration", reason="revise", pipeline_dir=tmp_path, now=BASE,
        )
    with pytest.raises(PersianVideoWorkflowError, match="next phase"):
        record_phase_attempt("run", "prepare_narration", pipeline_dir=tmp_path, now=BASE)


def test_cli_bootstrap_input_parsing_covers_supported_classes():
    parser = workflow.build_parser()
    cases = [
        (["--narration", "/tmp/n.wav"], ("narration_path", "/tmp/n.wav")),
        (["--script", "متن"], ("script", "متن")),
        (["--raw-text", "خام"], ("raw_text", "خام")),
        (["--english-article", "Article"], ("english_article", "Article")),
    ]
    for flags, expected in cases:
        args = parser.parse_args(["bootstrap", "--title", "Run", *flags])
        assert workflow._bootstrap_input(args) == expected
    with pytest.raises(SystemExit):
        parser.parse_args([
            "bootstrap", "--title", "Run", "--script", "x", "--raw-text", "y"
        ])


def test_front_door_skill_is_thin_and_links_canonical_contracts():
    front = (ROOT / "skills" / "persian-video" / "SKILL.md").read_text(encoding="utf-8")
    for flag in ("--narration", "--script-file", "--raw-text-file", "--english-article-file"):
        assert flag in front
    for capability in (
        "source-to-persian-narration.md",
    ...
    asset_director = (
        ROOT / "skills" / "pipelines" / "persian-footage" / "asset-director.md"
    ).read_text(encoding="utf-8")
    assert '"output_dir": str(project_dir / "assets"),' in asset_director
    assert '"output_dir": str(project_dir / "assets" / "clips"),' not in asset_director
