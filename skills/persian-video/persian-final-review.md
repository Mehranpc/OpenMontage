...
The final non-human review must record `retention_audit`, `post_render_motion_qa`, `silent_watch_audit`, `cut_rhythm`, `caption_readability`, `strongest_scene`, `weakest_scene`, `hook_strength`, and `resolution_strength`. Watch the candidate muted for the silent-watch fields; do not infer comprehension from subtitle presence or timeline metadata. `retention_audit.problems` must be empty and `post_render_motion_qa.passed` must be true before semantic review can advance. The motion audit samples the finished MP4 pixels; its long near-static runs are evidence about rendered pixels, not a relabeling of authored-timeline facts. Silent-watch main point/hook direction/conclusion must all be understood, and hook/resolution strength must be at least `acceptable` before the candidate can advance.

## Unified observable-quality evidence

Use `lib.persian_quality_evidence.compose_quality_evidence` whenever opening or final review combines retention, rendered motion, and cold-view semantics. Preserve the three evidence domains exactly: `authored_timeline`, `rendered_pixels`, and `semantic_review`. A report may say “no authored post-start shot/reveal change” while also saying “rendered intra-shot pixel motion observed”; those facts have different provenance and are not contradictory. Conversely, authored change with no measured rendered-pixel motion is a render-mismatch warning, not proof that the authoring metadata was false.

New render reports should persist `quality_evidence` composed from `retention_audit` + `post_render_motion_qa`. Legacy reports may omit it. Final-review validation recomputes the render-time form and rejects drift before layering `hookQualityReview.coldViewer` semantics into durable workflow `qualityEvidence`. Opening-review artifacts should likewise carry their opening motion audit plus `qualityEvidence` composed with the same vocabulary. Never turn deterministic motion measurement into semantic hook approval, and never treat timeline labels as proof that pixels changed.
...
The artifact contract enforces this in two layers. The persisted `hookQualityAudit` must itself record canonical user authority (`mode: "user_supplied"`, `authoritative: true`, and a `selectedHookSha256` that the review's `authorityProvenance` matches), so a hand-written checkpoint or review cannot lift the timing gate on its own word. The Persian workflow then re-derives the authority from the durable `hook_selection` record before presentation, and approval reconciliation re-runs the same check.
