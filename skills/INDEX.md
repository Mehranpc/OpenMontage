...
**Fresh-session production front door:** for an actual Persian video production starting from narration audio, an approved Persian script, or both, begin with `skills/persian-video/SKILL.md`. Text-only writing/rewriting does not create an OpenMontage project. The front door creates/resumes the bounded production envelope while the director files below remain the canonical stage-execution contracts.
