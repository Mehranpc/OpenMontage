...
- **Use:** `python -m lib.persian_video_workflow assets build-manifest <project-id> [--overrides-json <path>]`, then `assets write-checkpoint <project-id> [--review-json <path>] [--metadata-json <path>]`; use `assets music search` and `assets music fetch` for cached music acquisition. Never write `.workspace/*.py` for standard work.
